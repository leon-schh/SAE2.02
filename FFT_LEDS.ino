...
double vImag[SAMPLES]; //create vector of size SAMPLES to hold imaginary values
   ...
   bool inRange(int val, int minimum, int maximum) //pour faire un intevale
{
...
void setup() 
{
  ...
  pinMode(LED330, OUTPUT);
  pinMode(LED440, OUTPUT);
...
{  
  ...
  if ( inRange(peak, 215, 227)){
    ...
    if ( inRange(peak, 325, 342)){
    digitalWrite(LED330, HIGH);
  }
  else{
    digitalWrite(LED330, LOW);
  }

      if ( inRange(peak, 445, 455)){
    digitalWrite(LED440, HIGH);
  }
  else{
    digitalWrite(LED440, LOW);
  }

        if ( inRange(peak, 558, 565)){
