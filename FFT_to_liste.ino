...
const int maxValues = 10;  // Nombre maximal de valeurs dans le tableau
int sensorValues[maxValues];  // Tableau pour stocker les valeurs
int sensorValue;
int currentIndex = 0;  // Indice courant dans le tableau
...
bool isValueInArray(int value) {
  // Vérifier si la valeur est déjà présente dans le tableau
  for (int i = 0; i < maxValues; i++) {
    if (sensorValues[i] == value) {
      return true;  // La valeur est déjà présente
    }
  }
  return false;  // La valeur n'est pas présente
}
...
{  
    ...
    Serial.println(int(peak));     //Print out the most dominant frequency.
    sensorValue = int(peak);
if(inRange(peak, 147, 150) || inRange(peak, 166, 169) || inRange(peak, 175, 179)|| inRange(peak, 197, 202) || inRange(peak, 222, 227) || inRange(peak, 250, 255) || inRange(peak, 267, 269) || inRange(peak, 295, 305) || inRange(peak, 335, 338))
{
if (!isValueInArray(sensorValue)) {
    // La valeur n'est pas déjà présente dans le tableau, l'ajouter
    sensorValues[currentIndex] = sensorValue;
    currentIndex = (currentIndex + 1) % maxValues;  // Passer à l'indice suivant (circulaire)
  }

  // Afficher les valeurs actuelles du tableau
  for (int i = 0; i < maxValues; i++) {
